		...
		$displayOrderQuery =  "
			...
			if ($orderResult = $sqlconnection->query($displayOrderQuery)) {
				...
				if ($orderResult->num_rows == 0) {
					...
					echo "<tr><td class='text-center' colspan='7' >Sem pedidos agora :) </td></tr>";
				}
						...
						if ($currentspan == $rowspan) {
							...
							switch ($orderRow['status']) {
								case 'esperando':
									...
									echo "<button onclick='editStatus(this,".$orderRow['orderID'].")' class='btn btn-outline-primary' value = 'em preparo'>Preparando</button>";
									echo "<button onclick='editStatus(this,".$orderRow['orderID'].")' class='btn btn-outline-success' value = 'ok'>Pronto</button>";
								...
								case 'ok':
									...
									echo "<button onclick='editStatus(this,".$orderRow['orderID'].")' class='btn btn-outline-warning' value = 'finish'>Finalizado</button>";
	...
	if ($_GET['cmd'] == 'currentready') {
		...
		if ($result = $sqlconnection->query($latestReadyQuery)) {
			...
			if ($result->num_rows == 0) {
				echo "<tr><td class='text-center'>Sem pedido a ser servido. </td></tr>";
			}

            while($latestOrder = $result->fetch_array(MYSQLI_ASSOC)) {
            	echo "<tr><td><i class='fas fa-bullhorn' style='color:green;'></i><b> Pedido #".$latestOrder['orderID']."</b> pronto pra servir.<a href='editstatus.php?orderID=".$latestOrder['orderID']."'><i class='fas fa-check float-right'></i></a></td></tr>";
            }
        }
	}
